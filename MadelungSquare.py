...
def dist( x,y ): #Calculates the distance of (x,y) from (0,0)
...
def ML(n): #Calculates the madelung constant for a square lattice of side 2*n
    ...
    for i in range(-n,n): #Loops through x values
        for j in range(-n,n): #Loops through y values
            if i != 0 or j != 0: #Ignores the center point
                mdlung += pow(-1,1+i+j)/dist(i,j) #Adds contribution from each point
    return mdlung
